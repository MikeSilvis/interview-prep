...
func check(_ label: String, _ actual: Double, _ expected: Double) {
    let pass = abs(actual - expected) < 0.0001
    print("\(pass ? "✅ PASS" : "❌ FAIL") \(label): got \(actual), expected \(expected)")
}

let solution = Solution()
check("Test 1", solution.findMedianSortedArrays([1, 3], [2]), 2.0)
check("Test 2", solution.findMedianSortedArrays([1, 2], [3, 4]), 2.5)
check("Test 3", solution.findMedianSortedArrays([0, 0], [0, 0]), 0.0)
check("Test 4", solution.findMedianSortedArrays([], [1]), 1.0)
