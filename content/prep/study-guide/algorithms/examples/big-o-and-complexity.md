...
Use this chapter as your **mental ruler** for judging whether an idea is fast enough and roughly how it will behave as inputs grow. The goal is not to do formal proofs, but to be fluent in back‑of‑the‑envelope reasoning while you talk through designs.
...
- As you solve problems, **say the complexity out loud** and write it next to your solution. Force yourself to commit to a rough bound.
- When reading code at work or in reviews, mentally label common patterns: “single pass → \(O(n)\)”, “sort then scan → \(O(n \log n)\)”, “double nested loop → \(O(n^2)\)”.
- Pick a few typical `n` values (`10^3`, `10^5`, `10^6`) and practice estimating **operation counts** to build intuition about what’s feasible.
...
- **Hiding large constants behind “\(O(1)\)”**
  - A hash map lookup is \(O(1)\) on paper, but a huge constant factor (serialization, network, allocations) matters in real systems. Be willing to say “the Big‑O is fine, but the constant here is scary”.
- **Accidentally quadratic work**
  - Nested loops over the same collection, repeated scans inside a loop, or sorting inside a loop are common sources of accidental \(O(n^2)\) behavior. Train yourself to notice “loop inside loop” patterns.
- **Ignoring space complexity**
  - Building extra arrays, maps, or recursion stacks can be the difference between a feasible design and an out‑of‑memory incident. Always ask “What’s the **extra memory** this approach needs?”
- **Confusing worst‑case and average‑case**
  - Many structures (hash maps, quicksort) have different average vs worst‑case behavior. In interviews, state which one you’re assuming and why it’s reasonable.
- **Over‑engineering for tiny inputs**
  - For very small `n`, a simple \(O(n^2)\) algorithm may be better than a complex \(O(n \log n)\) one. Senior engineers call this out explicitly when arguing for maintainability.
- **Miscounting across multiple phases**
  - If you have “sort then scan” or “build map then traverse”, the total complexity is the **sum** of the phases, dominated by the largest term; don’t forget earlier passes just because the last step feels most important.
