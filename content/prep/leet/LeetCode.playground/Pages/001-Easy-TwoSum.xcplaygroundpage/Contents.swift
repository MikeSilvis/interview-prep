...
func check(_ label: String, _ actual: [Int], _ expected: [Int]) {
    let pass = Set(actual) == Set(expected)
    print("\(pass ? "✅ PASS" : "❌ FAIL") \(label): got \(actual), expected \(expected)")
}

let solution = Solution()
check("Test 1", solution.twoSum([2, 7, 11, 15], 9), [0, 1])
check("Test 2", solution.twoSum([3, 2, 4], 6), [1, 2])
check("Test 3", solution.twoSum([3, 3], 6), [0, 1])
